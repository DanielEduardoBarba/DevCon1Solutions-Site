    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-[60px]">
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 whitespace-nowrap ${
                  isActive
                    ? (isMarkour ? 'mk-nav-link-active' : isMarksman ? 'ms-nav-link-active' : 'bg-white/15 text-white')
                    : (isMarkour
                      ? 'mk-nav-link-idle text-white/60'
                      : isMarksman
                      ? 'ms-nav-link-idle text-white/60'
        ...
        <div className="lg:hidden" ref={ref}>
          ...
          {showMenu && (
            <div className="absolute top-[60px] right-4 w-56 py-2 menu-enter rounded-2xl border" style={{ background: isMarkour ? 'rgba(6,18,31,0.9)' : isMarksman ? 'rgba(18,6,7,0.9)' : 'rgba(10,10,26,0.85)', borderColor: isMarkour ? 'rgba(45,156,240,0.2)' : isMarksman ? 'rgba(255,77,61,0.2)' : 'rgba(255,255,255,0.1)', backdropFilter: 'blur(40px) saturate(1.5)', WebkitBackdropFilter: 'blur(40px) saturate(1.5)' }}>
              {menuOptions.map((opt, i) => (
                <NavLink
                  key={i}
                  to={opt.path}
                  ...
                  className={({ isActive }) =>
                    `block px-5 py-3 text-sm font-medium transition-colors ${
                      isActive
                        ? (isMarkour ? 'text-white bg-[rgba(45,156,240,0.18)]' : isMarksman ? 'text-white bg-[rgba(255,77,61,0.18)]' : 'text-white bg-white/10')
                        : (isMarkour ? 'text-white/60 hover:text-white hover:bg-[rgba(45,156,240,0.1)]' : isMarksman ? 'text-white/60 hover:text-white hover:bg-[rgba(255,77,61,0.1)]' : 'text-white/60 hover:text-white hover:bg-white/5')
                    }`
                  }
                >
                  {opt.label}
